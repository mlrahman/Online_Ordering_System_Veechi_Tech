<?php
	...
	if(isset($_REQUEST['give_food_order_search_list']) && isset($_REQUEST['order_id']))
	{
		...
		$search_value=trim($_REQUEST['search_value']);
				...
				{
					if($order_id!=0)
						$stmt=$conn->prepare("select * from order_info where order_id='$order_id' order by order_id desc ");
					else
						$stmt=$conn->prepare("select * from order_info where order_id LIKE '%$search_value%' OR customer_id IN(select customer_id from customer where first_name LIKE '%$search_value%' OR last_name LIKE '%$search_value%')  order by order_id desc ");
					...
					<div class="w3-medium w3-topbar w3-bottombar w3-container w3-border w3-padding-small w3-white w3-round w3-border-black" style="margin: 0px 0px 12px 0px;">
						<div class="w3-row">
							...
							<div class="w3-col " style="width:25%;">
								<a id="search_btn_<?php echo $row['order_id']; ?>" onclick="document.getElementById('search_btn_<?php echo $row['order_id']; ?>').style.display='none';document.getElementById('search_btn_hide_<?php echo $row['order_id']; ?>').style.display='block';document.getElementById('search_details_<?php echo $row['order_id']; ?>').style.display='block';" class="w3-button w3-green w3-round w3-tiny" style="padding:2px 4px;width:40px;margin-right:8px;float:left;">Details</a>
								<a id="search_btn_hide_<?php echo $row['order_id']; ?>" onclick="document.getElementById('search_btn_<?php echo $row['order_id']; ?>').style.display='block';document.getElementById('search_btn_hide_<?php echo $row['order_id']; ?>').style.display='none';document.getElementById('search_details_<?php echo $row['order_id']; ?>').style.display='none';" class="w3-button w3-red w3-round w3-tiny" style="padding:2px 4px;display:none;width:40px;margin-right:8px;float:left;">Hide</a>
								<?php
									if($row['status']=="In Queue")
									{
								?>
								...
								<?php
									...
									{
								?>
										<a class="w3-button w3-purple w3-round w3-tiny" style="padding:2px 4px;width:40px;margin-right:8px;float:left;" onclick="general_print_token('search_details_',<?php echo $row['order_id']; ?>)"><i class="fa fa-print"></i> Print</a>
										<i class="fa fa-check-square-o w3-text-blue" style="padding:2px 4px;margin-right:8px;float:left;cursor:pointer;" title="Order Delivered Successfull" onclick="delivery_done(<?php echo $row['order_id']; ?>);search_refresh(<?php echo $order_id; ?>,<?php echo $search_value; ?>);"></i>
										<i class="fa fa-close w3-text-red" style="padding:2px 4px;margin-right:8px;float:left;cursor:pointer;" title="Order Cancelled" onclick="cancel_done(<?php echo $row['order_id']; ?>);search_refresh(<?php echo $order_id; ?>,<?php echo $search_value; ?>);"></i>
								<?php
									}
									else
									{
						...
						<!-- Order Details -->
						<div id="search_details_<?php echo $row['order_id']; ?>" class="w3-pale-red w3-round w3-container w3-margin-top w3-border w3-padding-small" style="display:none;">
							<!-- Order date & time -->
							<div class="w3-row">
			...
			<?php
				...
				if($sl==0)
					echo '<p style="color:red;padding-top:120px;text-align:center;">No order added yet</p>';
