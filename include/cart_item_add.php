<?php
	...
	if(isset($_GET['id']) and isset($_GET['do_it']) and isset($_SESSION['cart_item']) and isset($_SESSION['cart_item_quantity']))
	{
		...
		{
			//Item need to add
			$_SESSION['cart_item'][]=$food_id;
			$_SESSION['cart_item_quantity'][$food_id]=1;
		}
		try
		{
			$stmt2 = $conn->prepare("select * from food where status='active' AND food_id='$food_id' order by food_id asc ");
			$stmt2->execute();
...
?>
		...
		<div class="w3-row w3-border-bottom" id="cart_item_<?php echo $item; ?>" onmouseover="show_cart_option('<?php echo $item; ?>')" onmouseout="hide_cart_option('<?php echo $item; ?>');" style="cursor:pointer;">
			...
			<p id="food_price_<?php echo $item; ?>" style="margin:0px;padding:0px;display:none;"><?php echo $food_price; ?></p>
