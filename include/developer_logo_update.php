<?php
	...
	if(isset($_REQUEST['update_logo']) && isset($_REQUEST['image']))
	{
		$link=$_REQUEST['image'];
		$file=$_FILES[$link];
		$image_name=photo_upload($file,0,100000,"jpg,gif,png,jpeg,bmp,heic",'../images/system',$path='');
		
		if($image!="1")
		{
			$mssg=makeThumbnails('../images/system/', $image_name, '' , '../images/system/', 375, 147); //width than height
			if($mssg=="done")
			{
